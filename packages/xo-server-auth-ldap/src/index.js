...
`.trim(),
    ...
    userIdAttribute: {
      title: 'ID attribute',
      description: 'Attribute used to map LDAP user to Nephora Conductor user. Must be unique. e.g.: `dn`',
      type: 'string',
    },
    groups: {
      ...
      properties: {
        ...
        idAttribute: {
          title: 'ID attribute',
          description: 'Attribute used to map LDAP group to Nephora Conductor group. Must be unique. e.g.: `gid`',
          type: 'string',
        },
        displayNameAttribute: {
          title: 'Display name attribute',
          description: "Attribute used to determine the group's name in Nephora Conductor. e.g.: `cn`",
          type: 'string',
        },
        membersMapping: {
    ...
    try {
      ...
      for (const ldapGroup of ldapGroups) {
        ...
        for (const memberId of ldapGroupMembers) {
          ...
          if (xoUser === undefined) {
            logger.debug(
              `LDAP user ${ldapUser[this._userIdAttribute]} belongs to group ${groupLdapName} but the corresponding Nephora Conductor user could not be found`
            )
            continue
          }
          ...
          if (userIdIndex !== -1) {
            logger.debug(
              `LDAP user ${ldapUser[this._userIdAttribute]} belongs to group ${groupLdapName} and is already a member of the corresponding Nephora Conductor group ${xoGroup.name}`
            )
