...
This line makes sure Cloudbase-init uses the NoCloud configuration drive (as generated by NC) as its data source.
...
- If you are Sysprepping an installation containing the XenServer/NCE PV drivers, you may need to set the `PersistAllDeviceInstalls` option in your `Unattend.xml` (Cloudbase-init's `Unattend.xml` already includes this option).
