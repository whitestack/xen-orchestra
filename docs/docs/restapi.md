...
While it's not yet at feature parity with the JSON-RPC API, you can explore all available endpoints through the [autogenerated Swagger documentation](./restapi.md#openapiswagger) included in your Nephora Conductor instance. Our goal is to make the REST API the main interface in the future, so it's the most future-proof choice going forward.
...
As part of our continuous efforts to improve the developer experience, we’ve begun documenting the Nephora Conductor REST API using the OpenAPI (Swagger) standard:
...
### Accessing the Nephora Conductor API documentation in Swagger
